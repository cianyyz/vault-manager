...
describe("whirlpool-cpi-sample", () => {
  ...
  it("load whirlpools config account", async () => {
    ...
    const signature = await program.methods.verifyWhirlpoolsConfigAccount()
      .accounts({
        signer_account: wallet.publicKey,
  ...
  it("load fee tier 1 account", async () => {
    ...
    const signature = await program.methods.verifyFeetierAccount()
      .accounts({
        signer_account: wallet.publicKey,
        feetier: feetier_pubkey
      })
      .rpc();
    await connection.confirmTransaction(signature, "confirmed");
  ...
  it("execute proxy swap SOL to USDC", async () => {
    ...
    const transaction = new TransactionBuilder(connection, wallet, transaction_builder_opts)
      ...
      .addInstruction({instructions: [swapIx], cleanupInstructions: [], signers: []});
  ...
  it("execute proxy collect_fees", async () => {
    ...
    const collect_fees = await program.methods.proxyCollectFees()
      .accounts({
        signer_account: wallet.publicKey,
        whirlpoolProgram: ORCA_WHIRLPOOL_PROGRAM_ID,
        whirlpool: samo_usdc_whirlpool_pubkey,
