...
pub struct ProxyOpenPosition<'info> {
  ...
  #[account(
    mut,
    constraint = (
      // Either both position and whirlpool are empty (default)
      (vault.current_position == None && 
       vault.current_whirlpool == None) ||
      // Or they match the position and whirlpool being operated on
      (vault.current_position == Some(position.key()) && 
       vault.current_whirlpool == Some(whirlpool.key()))
    ) @ VaultError::PositionMismatch
  )]
